...
const (
	...
	passwd             = "zebra\n"                      // BGP vty password
	showBgpIpv4Sum     = "show bgp ipv4 summary json\n" // show bgp ipv4 summary vty command
	showBgpIpv6Sum     = "show bgp ipv6 summary json\n" // show bgp ipv6 summary vty command
...
// run_command runs list of vty commands
func (m BGPSummaryMessage) run_command(poller *BgpPoller, cmd_key string) {
	commands := []string{showBgpIpv4Sum, showBgpIpv6Sum}
	for _, cmd := range commands {
		m.processCommand(poller, cmd_key, cmd)
	}
}

// processCommand sends bgp vty commands to socket and process responses
func (m BGPSummaryMessage) processCommand(poller *BgpPoller, cmd_key string, cmd string) {
